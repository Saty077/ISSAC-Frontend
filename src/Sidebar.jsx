...
function Sidebar() {
  ...
  const getAllThreads = async () => {
    const response = await fetch(
      "http://ec2-3-7-54-1.ap-south-1.compute.amazonaws.com:8080/api/thread",
    );
  ...
  const changeThread = async (newThreadId) => {
    ...
    try {
      const response = await fetch(
        `http://ec2-3-7-54-1.ap-south-1.compute.amazonaws.com:8080/api/thread/${newThreadId}`,
      );
      const res = await response.json();
      setPrevChats(res);
      setNewChat(false);
  ...
  const deleteThread = async (newThreaId) => {
    try {
      const response = await fetch(
        `http://ec2-3-7-54-1.ap-south-1.compute.amazonaws.com:8080/api/thread/${newThreaId}`,
        { method: "DELETE" },
      );
      const res = await response.json();
      console.log(res);
